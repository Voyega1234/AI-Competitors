...
// Function to get top ads sorted by a specific metric
const getTopAdsByMetric = async (metric: string, order: 'asc' | 'desc' = 'desc', limit: number = 10, adAccountId?: string) => {
  try {
    // Validate metric to prevent SQL injection
    const validMetrics = ['ctr', 'cpc', 'roas', 'clicks', 'impressions', 'spend', 'frequency'];
    ...
    // Get all matching ads first
    const { data: ads, error } = await query;

    if (error) throw error;
    if (!ads || ads.length === 0) return [];

    // Convert string numbers to actual numbers and sort
    const processedAds = ads.map(ad => {
      const metricValue = ad[metric as keyof typeof ad];
      const numericValue = typeof metricValue === 'string' ? 
        parseFloat(metricValue.replace(/,/g, '')) : 
    ...
    // Sort the ads by the metric value
    ...
    return processedAds.slice(0, limit);
  } catch (error) {
    console.error(`Error fetching top ads by ${metric}:`, error);
    return [];
  }
};
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Try to get ad account ID from URL parameters first
    const clientName = searchParams.get('clientName');
    const productFocus = searchParams.get('productFocus');
    let adAccountId = searchParams.get('adAccountId');

    // If adAccountId is not provided, try to fetch it using clientName and productFocus
    if (!adAccountId && clientName && productFocus) {
        console.log(`[Ad Account] Fetching ad account ID for client: ${clientName}, product: ${productFocus}`);
        const { data: adAccountData, error: adAccountError } = await supabaseAdmin
            .from('AnalysisRun')
            .select('ad_account_id')
            .eq('clientName', clientName.trim())
            .eq('productFocus', productFocus.trim())
            .order('createdAt', { ascending: false })
            .limit(1)
            .single();

        if (!adAccountError && adAccountData?.ad_account_id) {
            adAccountId = adAccountData.ad_account_id;
            console.log(`[Ad Account] Found ad account ID: ${adAccountId} for client: ${clientName}, product: ${productFocus}`);
        } else {
            console.error('[Ad Account] Error fetching ad account ID:', adAccountError || 'No ad account found');
        }
    }
...
Your response should be valid, parseable JSON with a "recommendations" array, matching the exact original structure.`;
                ...
                // Use GPT's improved output as the final result
                console.log("Using GPT's improved output as the final result.");
                finalResults['gemini'] = gptImprovedOutput;
                
                // Set the model competition flag to true but with only two steps
...
` : '';
            ...
            const defaultDetailsText = `a.  **\`content_pillar\`:** กำหนดธีมเนื้อหาหลักหรือหมวดหมู่ **(ภาษาไทย)** (เช่น "เคล็ดลับฮาวทู", "เบื้องหลังการทำงาน", "เรื่องราวความสำเร็จลูกค้า", "การหักล้างความเชื่อผิดๆ", "ไลฟ์สไตล์และการใช้งาน", "ปัญหาและการแก้ไข").
                                ...
                                c.  **\`concept_idea\`:** สรุปแนวคิดสร้างสรรค์หลัก (1-3 ประโยค) สำหรับการนำเสนอไอเดียนี้ **(ภาษาไทย)**.
                                d.  **\`copywriting\`:** สร้างสรรค์องค์ประกอบข้อความโฆษณาเบื้องต้น **(ภาษาไทย)**:
                                    *   **\`headline\`:** พาดหัวที่ดึงดูดความสนใจ **(ภาษาไทย)**.
                                    *   **\`sub_headline_1\`:** พาดหัวรองที่ขยายความหรือเน้นประโยชน์ **(ภาษาไทย)**.
                                    *   **\`sub_headline_2\`:** พาดหัวรองที่สอง (ถ้ามี) เพื่อเพิ่มบริบทหรือความเร่งด่วน **(ภาษาไทย)** (ใช้ \`null\` หากไม่ต้องการ).
